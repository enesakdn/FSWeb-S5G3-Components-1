...
function haberYapici(haber) {
  ...
  divolustur.setAttribute("class", "article");

  const h2olustur = document.createElement("h2");
  h2olustur.textContent = haber.baslik;

  const polustur = document.createElement("p");
  polustur.setAttribute("class", "tarih");
  polustur.textContent = haber.tarih;

  const p1 = document.createElement("p");
  p1.textContent = haber.ilkparagraf;

  const p2 = document.createElement("p");
  p1.textContent = haber.ikinciparagraf;

  const p3 = document.createElement("p");
  p1.textContent = haber.ucuncuparagraf;

  const spanolustur = document.createElement("span");
  spanolustur.setAttribute("class", "expandButton");
  spanolustur.textContent = "+";

  spanolustur.addEventListener("click", () =>
    divolustur.classList.toggle("article-open")
  );

  divolustur.appendChild(h2olustur);
  divolustur.appendChild(polustur);
  divolustur.appendChild(p1);
  divolustur.appendChild(p2);
  divolustur.appendChild(p3);
  divolustur.appendChild(spanolustur);

  return divolustur;
}

data.forEach((a) => {
  document.querySelector(".articles").appendChild(haberYapici(a));
});
